...
#define HIDD_DEVICE_NAME            "HID_TEST"
...
void move_mouse_task(void *arg) {
    while (1) {
        if (sec_conn) {
            // Move mouse right
            ESP_LOGI(HID_DEMO_TAG, "Mouse moving right");
            ...
            // Move mouse left
            ESP_LOGI(HID_DEMO_TAG, "Mouse moving left");
            esp_hidd_send_mouse_value(hid_conn_id, 0, -50, 0);
            vTaskDelay(pdMS_TO_TICKS(100));
            ...
            vTaskDelay(pdMS_TO_TICKS(1000)); // Check again in a second if not connected
        }
    }
}
...
void app_main(void)
{
    ...
    xTaskCreate(&move_mouse_task, "mouse_task", 2048, NULL, 5, NULL);
